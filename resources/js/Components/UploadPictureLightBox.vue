<script setup>
    ...
    try {
        const checkResponse = await axios.get('/api/v1/pictures?name=' + pic.value.name + '&houseId=' + pic.value.houseId + '&type=H');
    ...
    async function getPhotos(){
        let url = '/api/v1/pictures?houseId=' + props.houseId + '&type=H';
        try{
    ...
    const closeUploadPictureLightBox = () => {
        emits('closeUploadPictureLightBox', false);
    };
    ...
    <div class="uploadPictureLightBox">
        <div class="formContainer">
            ...
            <div class="savedPictures">
                ...
                <button class="doneBtn" @click="closeUploadPictureLightBox">Done</button>
            </div>
        </div>
    </div>
</template>
<style scoped>
.uploadPictureLightBox{
    position: fixed;
    top: 0;
    left: 0;
    display: flex;
