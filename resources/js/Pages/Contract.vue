<script setup>
    ...
    onMounted(() => {
        ...
        pic.value.name = props.houseId + '_contract'; 

        // 若已有pic，將照片顯示於畫面中
        let picUrl = '/api/v1/pictures?houseId=' + props.houseId + '&type=C';
        console.log(picUrl);
        axios.get(picUrl)
        .then((response) => {
    ...
    async function uploadPhoto() {
        try {
            const checkResponse = await axios.get('/api/v1/pictures?name=' + pic.value.name + '&houseId=' + pic.value.houseId + '&type=C');
            // 若照片已存在
            if (checkResponse.data.data.length) { 
                console.log(checkResponse);
                let replace = confirm("Contract has existed. Replace?");
                if(replace){
    ...
    function completeProgressTwo(){
        axios.patch('/api/v1/projects/'+ pic.value.houseCode, {progress : 2})
        .then(() => {
            // 成功變回到progress頁面
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="contract">
            <h2> <img :src="contractIcon">Upload Signed Contract</h2>
            <div class="form">
            ...
            <div class="btns">
                ...
                <button @click="completeProgressTwo">Complete</button>
            </div>
        </div>
    </AuthenticatedLayout>
</template>
